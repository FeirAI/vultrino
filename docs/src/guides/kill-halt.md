...
2. **Install an authoritative per-agent kill policy** (`principal_pattern` = the agent label). This covers API-key-authed agents that carry no use token. It is a **kill** policy: unlike an ordinary per-agent Deny, it is evaluated *before* every other policy, so an `allow` rule that happens to be ordered first can never let a halted agent slip through. It is persisted and propagates to other processes via the policy refresh.

...
pub fn derive_secret_forms(secrets: &[Zeroizing<String>]) -> Vec<String> {
    ...
    for secret in secrets {
        ...
        // reflected by such an upstream would evade the default JSON-escaped form. Emitted
        // in BOTH hex cases (\uXXXX hex is case-insensitive; encoders are consistent-case),
        // each also COMPOSED with `/` → `\/` (an ensure_ascii + HTML-safe encoder applies
        // both transforms to a secret with non-ASCII AND a slash).
        if !raw.is_ascii() {
            for upper in [false, true] {
                let ascii = ascii_escaped_inner(raw, upper);
                if ascii.contains('/') {
...
/// `upper` selects the hex-digit case (both are generated by the caller).
fn ascii_escaped_inner(s: &str, upper: bool) -> String {
    let u = |cp: u32| {
        if upper {
            format!("\\u{:04X}", cp)
        } else {
